...
const workloadName = computed(() => {
  if (['Deployment', 'StatefulSet', 'DaemonSet', 'ReplicaSet'].includes(props.resourceKind) && props.resourceData?.metadata?.name) {
...
const storageKey = computed(() => `kide-workload-pods-column-sizes`)
...
async function refreshPods(): Promise<void> {
  ...
  try {
    ...
    const podList = await invoke<any[]>('get_pods_by_selector', {
      namespace: props.resourceData.metadata.namespace,
      selector
    })
    ...
    error.value = err.message || 'Failed to fetch pods for this workload'
    pods.value = []
  } finally {
    loading.value = false
  }
...
function getSelector(): string {
  if (!getGenericSpec(props.resourceData)?.selector) return ''

  if (props.resourceKind === 'DaemonSet' || props.resourceKind === 'Deployment' || props.resourceKind === 'ReplicaSet') {
    const matchLabels = getGenericSpec(props.resourceData).selector.matchLabels
    if (matchLabels) {
  ...
  if (props.resourceKind === 'StatefulSet') {
    const matchLabels = getGenericSpec(props.resourceData).selector.matchLabels
    if (matchLabels) {
      return Object.entries(matchLabels)
        .map(([key, value]) => `${key}=${value}`)
...
function getReadyContainers(pod: any): number {
  if (!getGenericStatus(pod)?.containerStatuses) return 0
  return getGenericStatus(pod).containerStatuses.filter((cs: any) => cs.ready).length
}

function getTotalContainers(pod: any): number {
  return getGenericStatus(pod)?.containerStatuses?.length || 0
}
...
watch(() => props.resourceData, (newResource) => {
  if (newResource && ['Deployment', 'StatefulSet', 'DaemonSet', 'ReplicaSet'].includes(props.resourceKind)) {
    refreshPods()
  }
}, { immediate: true })
...
// Initialize
onMounted(() => {
  if (props.resourceData && ['Deployment', 'StatefulSet', 'DaemonSet', 'ReplicaSet'].includes(props.resourceKind)) {
    refreshPods()
  }
})
</script>