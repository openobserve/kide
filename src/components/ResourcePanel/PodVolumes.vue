<template>
  <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
    <h3 class="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-3">
      Volumes
      <span class="text-xs font-normal text-gray-500 dark:text-gray-400 ml-2">({{ volumes.length }})</span>
    </h3>
    <div v-if="volumes.length === 0" class="text-sm text-gray-500 dark:text-gray-400">
        ...
        <div class="flex items-start justify-between gap-2">
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 mb-2">
              ...
              <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 dark:bg-gray-900/30 text-gray-700 dark:text-gray-300">
            ...
            <div class="text-xs space-y-1">
              ...
              <div v-if="volume.configMap" class="space-y-1">
                ...
                <div v-if="volume.configMap.defaultMode" class="text-gray-600 dark:text-gray-400">
                  <span class="font-medium">Default Mode:</span> {{ formatFileMode(volume.configMap.defaultMode) }}
                </div>
                <div v-if="volume.configMap.items?.length" class="text-gray-600 dark:text-gray-400">
              ...
              <div v-else-if="volume.secret" class="space-y-1">
                ...
                <div v-if="volume.secret.defaultMode" class="text-gray-600 dark:text-gray-400">
                  <span class="font-medium">Default Mode:</span> {{ formatFileMode(volume.secret.defaultMode) }}
                </div>
                <div v-if="volume.secret.items?.length" class="text-gray-600 dark:text-gray-400">
                  <span class="font-medium">Items:</span>
                  <div class="ml-2 mt-1 space-y-1">
              ...
              <div v-else-if="volume.emptyDir" class="space-y-1">
                <div v-if="volume.emptyDir.medium" class="text-gray-600 dark:text-gray-400">
                  <span class="font-medium">Medium:</span> {{ volume.emptyDir.medium }}
                </div>
                <div v-if="volume.emptyDir.sizeLimit" class="text-gray-600 dark:text-gray-400">
              ...
              <div v-else-if="volume.hostPath" class="space-y-1">
                ...
                <div v-if="volume.hostPath.type" class="text-gray-600 dark:text-gray-400">
              ...
              <div v-else-if="volume.downwardAPI" class="space-y-1">
                <div v-if="volume.downwardAPI.defaultMode" class="text-gray-600 dark:text-gray-400">
                  <span class="font-medium">Default Mode:</span> {{ formatFileMode(volume.downwardAPI.defaultMode) }}
                </div>
                <div v-if="volume.downwardAPI.items?.length" class="text-gray-600 dark:text-gray-400">
                  <span class="font-medium">Items:</span>
                  <div class="ml-2 mt-1 space-y-1">
              ...
              <div v-else-if="volume.projected" class="space-y-1">
                <div v-if="volume.projected.defaultMode" class="text-gray-600 dark:text-gray-400">
                  <span class="font-medium">Default Mode:</span> {{ formatFileMode(volume.projected.defaultMode) }}
                </div>
                <div v-if="volume.projected.sources?.length" class="text-gray-600 dark:text-gray-400">
                  <span class="font-medium">Sources:</span> {{ volume.projected.sources.length }} source(s)
                </div>
                
                <!-- Sources Subsection -->
                <div v-if="volume.projected.sources?.length" class="mt-2 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                  <h5 class="text-xs font-semibold text-blue-800 dark:text-blue-300 mb-2">Source Details</h5>
                  <div class="space-y-2">
                    <div v-for="(source, index) in volume.projected.sources" :key="index" class="p-2 bg-white dark:bg-gray-800 rounded border border-blue-200 dark:border-blue-700">
                      <!-- ConfigMap Source -->
                      <div v-if="source.configMap" class="space-y-1">
                        <div class="font-medium text-blue-600 dark:text-blue-400 text-xs">ConfigMap</div>
                        <div class="text-xs text-gray-600 dark:text-gray-400">
                          <span class="font-medium">Name:</span> {{ source.configMap.name }}
                        </div>
                        <div v-if="source.configMap.optional !== undefined" class="text-xs text-gray-600 dark:text-gray-400">
                          <span class="font-medium">Optional:</span> {{ source.configMap.optional ? 'Yes' : 'No' }}
                        </div>
                        <div v-if="source.configMap.items?.length" class="text-xs text-gray-600 dark:text-gray-400">
                          <span class="font-medium">Items:</span>
                          <div class="ml-2 mt-1 space-y-1">
                      ...
                      <div v-else-if="source.secret" class="space-y-1">
                        <div class="font-medium text-purple-600 dark:text-purple-400 text-xs">Secret Source</div>
                        <div class="text-xs text-gray-600 dark:text-gray-400">
                          <span class="font-medium">Name:</span> {{ source.secret.name }}
                        </div>
                        <div v-if="source.secret.optional !== undefined" class="text-xs text-gray-600 dark:text-gray-400">
                          <span class="font-medium">Optional:</span> {{ source.secret.optional ? 'Yes' : 'No' }}
                        </div>
                        <div v-if="source.secret.items?.length" class="text-xs text-gray-600 dark:text-gray-400">
                          <span class="font-medium">Items:</span>
                          <div class="ml-2 mt-1 space-y-1">
                      ...
                      <div v-else-if="source.downwardAPI" class="space-y-1">
                        <div class="font-medium text-green-600 dark:text-green-400 text-xs">Downward API</div>
                        <div v-if="source.downwardAPI.items?.length" class="text-xs text-gray-600 dark:text-gray-400">
                          <span class="font-medium">Items:</span>
                          <div class="ml-2 mt-1 space-y-1">
                      ...
                      <div v-else-if="source.serviceAccountToken" class="space-y-1">
                        <div class="font-medium text-orange-600 dark:text-orange-400 text-xs">Service Account Token</div>
                        <div class="text-xs text-gray-600 dark:text-gray-400">
                          <span class="font-medium">Path:</span> {{ source.serviceAccountToken.path }}
                        </div>
                        <div v-if="source.serviceAccountToken.audience" class="text-xs text-gray-600 dark:text-gray-400">
                          <span class="font-medium">Audience:</span> {{ source.serviceAccountToken.audience }}
                        </div>
                        <div v-if="source.serviceAccountToken.expirationSeconds" class="text-xs text-gray-600 dark:text-gray-400">
            ...
            <div v-if="getVolumeMounts(volume.name).length > 0" class="mt-3 pt-2 border-t border-gray-200 dark:border-gray-600">
              <div class="text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Mounted in:</div>
              <div class="space-y-1">
                <div v-for="mount in getVolumeMounts(volume.name)" :key="`${mount.containerName}-${mount.mountPath}`"
                  ...
                  <span class="font-mono text-gray-600 dark:text-gray-400">{{ mount.mountPath }}</span>
                  <span v-if="mount.readOnly" class="text-orange-600 dark:text-orange-400 text-xs">(RO)</span>
                  <span v-if="mount.subPath" class="text-gray-500 dark:text-gray-400 text-xs">subPath: {{ mount.subPath }}</span>
                </div>
              </div>
            </div>
          </div>
